...
export default function LaudeCard(laude: LAUDE, isText: boolean) {
  return (
    <motion.div
    ...
    >
      {isText && <p className="text-xl font-bold select-none">{laude}</p>}
      <Image
        priority
        draggable={false}
        ...
        height={150}
      />
      {isText && <p className="italic">{toLatin(laude)}</p>}
    </motion.div>
  );
}
